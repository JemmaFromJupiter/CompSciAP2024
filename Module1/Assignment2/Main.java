...
class Main {
  ...
  // <== This is used when the player decides they dont want either of the above
  // functions as hints ==> //
  ...
  // <== Sets the difficulty of the game by getting a value between 1 and 10, then
  // calculating the upper limit and how many turns the player has based on the
  // difficulty ==> //
  ...
  public static boolean play_game() {
    ...
    // <== Sets the difficulty and generates a random number from 1 to the upper
    // limit ==> //
    int[] difficulty = set_difficulty();
    int upperLimit = difficulty[0];
    int numTurns = difficulty[1];
    ...
    // <== For loop that breaks once the user has won, or when the user uses up all
    // of their tries. ==> //
    for (int turnsRemaining = numTurns; turnsRemaining >= 1; turnsRemaining--) {
      ...
      // <== If the user guesses the number correctly, the loop breaks and they have
      // won ==> //
      // <== Otherwise, the player gets feedback based on how hot/cold or high/low
      // they are or no feedback ==> //
      if (guess == number) {
    ...
    // <== Prompts the player to play again, returns true or false based on their
    // choice ==> //
