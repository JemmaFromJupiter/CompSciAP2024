...
public class AddStudent extends JFrame {
	 ...
	 * Create the frame.
	 */
	public AddStudent(StorageHandler shdl, RuntimeDatabase rdb, DefaultTableModel dtm) {
		...
		mainPanel.add(inputPanel, "cell 0 0 2 2,push ,grow,grow");
		...
		JComboBox<String> genderSelect = new JComboBox<>();
		...
		JComboBox<String> pronounsSelect = new JComboBox<>();
		...
		btnConfirm.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!inputEmail.getText().isBlank() && !inputFirstName.getText().isEmpty() && !inputLastName.getText().isEmpty() && genderSelect.getSelectedIndex() != -1) {
					...
					newStudent = new Student(
							...
							inputPrefFirst.getText() + " " + inputPrefLast.getText(),
							(String) genderSelect.getSelectedItem(),
							(String) pronounsSelect.getSelectedItem(),
							inputEmail.getText(),
							parseDateToString());
					
					rdb.append(newStudent);
					dtm.addRow(newStudent.asArray());
					try {
					...
					System.out.println("Not All Required Fields Are Filled.");
				}
			}
		});
